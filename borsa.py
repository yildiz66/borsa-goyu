...
# TÜM KATILIM LİSTESİ
KATILIM_TUMU = ["AKSA", "ALTNY", "ASELS", "BIMAS", "BSOKE", "CANTE", "CIMSA", "CWENE", "DOAS", "EGEEN", "ENJSA", "EREGL", "FROTO", "GENTS", "GESAN", "GUBRF", "HEKTS", "JANTS", "KCAER", "KONTR", "KONYA", "KORDS", "KOZAL", "MAVI", "MGROS", "MIATK", "OYAKC", "PGSUS", "REEDR", "SASA", "SISE", "SMRTG", "TABGD", "THYAO", "TKFEN", "TMSN", "TOASO", "TUPRS", "ULKER", "VESBE", "YEOTK"]
...
def set_commands():
    try:
        ...
        time.sleep(1)
        commands = [
            telebot.types.BotCommand("gunluk", "Günlük Tarama (Tümü)"),
...
def analiz_motoru(hisse, donem="1d"):
    try:
        ...
        # Yahoo Finance Retry (Hata verirse 2. kez dene)
        df = yf.download(f_t, period="2y", interval=donem, progress=False, timeout=15)
        if df is None or df.empty:
            df = yf.download(ticker, period="2y", interval=donem, progress=False) # .IS olmadan dene
        ...
        return {"ticker": ticker, "fiyat": float(df.iloc[-1]["Close"]), "rsi": float(df.iloc[-1]["RSI"]), "ust": float(bb.iloc[-1, 2]), "df": df}
    except: return None

def rapor_gonder(liste, vade_kod, baslik):
    bot.send_message(MY_ID, f"🔍 {baslik} için {len(liste)} hisse taranıyor...")
    havuz = []
    for h in liste:
        res = analiz_motoru(h, vade_kod)
        # RSI 30-75 arası olanları topla
    ...
    # Gemini Yorumu
    try:
        prompt = f"Borsa uzmanı olarak şu hisseleri {baslik} periyodu için yorumla: {ai_data}"
        bot.send_message(MY_ID, f"🤖 *Gemini Yorumu:* {ai_model.generate_content(prompt).text}")
    except: pass

@bot.message_handler(commands=['start'])
def start(m): bot.send_message(m.chat.id, "📈 Terminal Aktif Emir Bey. Tüm seçenekler menüye yüklendi.")
